...
是不是很简单，啥都不用你干，输入几个参数就完事了。但**需要注意的是，这里的待测点坐标需要转化成二维数组，不然是会报错的！！！knn.kneighbors\(\) 这个方法的返回值也是一个二维数组！！！设置了return\_distance这个参数为False之后，返回的值就是最近邻点的索引。如果没设置的话，会返回索引和距离**。
