...
其中出现了洗牌函数random\_card\(\)，进行每一局游戏函数every\_round\(\)，询问是否继续的函数continue\_game\(\)。

为了说明方便，就按 洗牌函数random\_card\(\)、询问是否继续的函数continue\_game\(\)、进行每一局游戏函数every\_round\(\)， 这样的顺序写吧。
...
还剩 进行每局游戏的函数every\_round\(\)。

该函数需要从主函数接收一个参数poker\_pool，即表示扑克牌的卡池。
...
叫牌函数get\_one\_card\(\)、分数计算函数score\_count\(\)、询问是否继续叫牌的函数if\_get\_next\(\)、庄家叫牌函数pc\_get\_card\(\)。

再依次进行具体的定义呗。

这里需要注意的是，因为在 pc\_get\_card\(\) 里面，我使用到了get\_one\_card\(\) 和 score\_count\(\) 这两个函数。所以这两个函数需要在pc\_get\_card\(\) 函数定义之前就先被定义好才行。

那就先写 叫牌函数get\_one\_card\(\) 呗。

该函数的作用就是从卡池里抽取一张卡，即从卡池里删除一张卡，并返回被删除的这张卡。

当然因为需要卡池这个变量，所以就需要传入一个参数poker\_pool，返回一个字符串，表示抽取的这样卡。

注意这里虽然对poker\_pool变量进行了修改，但不需要返回被修改后的变量，就可以达到改变原有的变量的目的。\(具体可以去看看Python的基础知识\)
...
然后再写 分数计算函数score\_count\(\)。

该函数的作用是计算手牌目前的点数，所以需要传入一个表示手牌的参数hand\_card，返回该手牌的点数。
...
最后就是 庄家叫牌函数pc\_get\_card\(\)。

该函数的作用就是庄家进行叫牌，叫牌规则之前已经说过，就不再赘述了。

因为需要调用get\_one\_card\(\)和score\_count\(\)，函数所以需要传入这两个函数所需要的参数poker\_pool和pc\_hand\_card。另外因为需要与玩家分数进行比较而决定是否叫牌，所以需要传入参数player\_score。
...
回到正题，修改的地方有两处，一是修改了 庄家叫牌函数pc\_get\_card\(\)，然后额外定义了一个用来计算概率的函数。
...
可以看到这里我多定义了一个函数win\_or\_equal\_posibility\(\)用来计算获胜和平局的概率。

也很简单，就是遍历卡池里现有的牌，统计抽取后能获胜和平局牌的数量，再计算总概率。

因此需要传入，庄家目前手牌点数pc\_score，玩家手牌点数player\_score，卡池poker\_pool这三个参数。
