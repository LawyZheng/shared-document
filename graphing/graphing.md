...
写到这里，我就忍不住想吐槽一下plotly这个库。它的图形有的在 plotly.graph\_objects 里，有的却在 plotly.express。也说不好哪个在哪，基本上我感觉是比较基础统计图（像是频率分布、折线、箱、散点都基本在plotly.express里），但建议大家看的懂官方文档的话，就去文档查一下，看不懂的话就花时间记一下。

另外还有一点就是在plotly.graph\_objects下的图都是大写字母开头，而在plotly.express下的图都是小写字母开头，这样的设计其实让人挺头疼的。
...
pandas读取csv文件，excel文件，都有对应的 pandas.read\_xxx 方法。记得对应上自己存数据的文件类型。
...
因为在我后面的数据处理中，没有用到 Stay\_In\_Current\_City\_Years 、Marital\_Status 、Product\_Category\_1 、 Product\_Category\_2 、 Product\_Category\_3 这些列里的数据，所以先把他们筛选除去。
...
pandas筛选数据的方法很多，这里用了pivot\_table进行筛选，用这个函数主要是因为它可以对数据进行分类重构。
...
**最后别忘了调用一下reset\_index\(\)方法！！**需要对DataFrame进行重新排列，不然得到的结果会是一个多重表格。
...
先用gender\_data.Gender.unique\(\)获取数据表中所有的性别种类，即F和M。然后就进入了for循环。

go.Bar\(\)函数用来绘制柱状图。需要绘制的图为 x轴代表Purchase, y轴代表Age，所以在参数里分别传入了 gender\_data.Purchase 和 gender\_data.Age。而对于gender\_data.Purchase我们需要进行筛选，因为两次绘制的分别是代表F和M的图表，所以在绘制的时候我们需要筛选出对应的Purchase数据。
...
先观看一下原始数据，可以发现每个人都有一个唯一的 User\_ID ,而一个人会购买多个不同的产品。所以需要整理出来的数据就是，对于每个相同的 User\_ID,都让 Purchase 列中的值进行累加，得到最终的数据就可以了。
...
因为需要根据性别绘制不同的统计图，所以在 index 参数里依然传入了 Gender。但事实上性别并不会影响最终的数据结果，因为一个 User\_ID 肯定只会对应一个性别的，毕竟一个人总不可能有两个性别吧。
...
回到目标上来，需要一个数据表，里面有每个人买了几件东西，花了多少钱，以及来自哪个城市的数据。所以需要对原始的列表，根据同一个 User\_ID 进行累加操作，得到每个人消费的金额数。再计算每个 User\_ID 都出现了几次，这样就可以算出每个人购买的数量了。
...
这样我们就得到了两个数据表，一个记录着消费金额，一个记录着购买数量，然后将这两个表横向融合起来就可以了。列表的横向融合一般都是基于有相同列的，在这里相同的列就是 User\_ID、Age、City\_Category。
...
可以看到，数据表中已经有了消费金额和购买数量两列数据。不过事实上这里可以通过pivot\_table的aggfunc参数进行一步到位的筛选。但因为想多普及一下pandas的基础知识，就用了merge的办法。
